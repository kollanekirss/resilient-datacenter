...
def test_independent_wizard_generates_infrastructure_and_two_manifests(tmp_path,monkeypatch):
    ...
    replies=iter(['independent','my-home','control.pilot.test','1.1.1.1','ubuntu','relay.pilot.test','8.8.8.8','ubuntu','lab-admin','/local/control.crt','/local/control.key','/local/relay.crt','/local/relay.key','/local/derper','a'*64,'2','home-services','tag:home-services','recovery-services','tag:recovery-services','yes'])
...
def test_missing_infrastructure_files_save_draft_not_ready_output(tmp_path):
    replies=iter(['independent','my-home','control.pilot.test','1.1.1.1','ubuntu','relay.pilot.test','8.8.8.8','ubuntu','lab-admin','/absent/control.crt','/absent/control.key','/absent/relay.crt','/absent/relay.key','/absent/derper','a'*64,'1','home-services','tag:home-services'])
    assert wizard().run_wizard(tmp_path,input_fn=lambda _:next(replies),output_fn=lambda _:None)=='draft'
    assert not (tmp_path/'infrastructure.yml').exists()

...
FILES=APPLICATION_FILES+tuple(name for name in ISSUER_FILES if name not in APPLICATION_FILES)+('gateway_backup.py','gateway_backup_runtime.py')
...
def verify_runtime(directory=RUNTIME,*,require_root=True):
    ...
    if set(data)!={'schema_version','files'} or data['schema_version']!=1 or not isinstance(data['files'],dict) or set(data['files']) not in (set(FILES),set(APPLICATION_FILES),set(MATRIX_FILES),set(LEGACY_FILES)):

...
def verify_runtime(directory=RUNTIME,*,require_root=True):
    ...
    if set(data)!={'schema_version','files'} or data['schema_version']!=1 or not isinstance(data['files'],dict) or set(data['files']) not in (set(FILES),set(MATRIX_FILES),set(LEGACY_FILES)):

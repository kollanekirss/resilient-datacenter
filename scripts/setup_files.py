...
def prepare_outputs(kind: str, configuration: dict) -> dict[str,str]:
    ...
    if kind=='independent':
        steps += ['On the operator workstation, from the project directory:', '```sh', './rdc infrastructure check /ABSOLUTE/PATH/infrastructure.yml', './rdc infrastructure apply /ABSOLUTE/PATH/infrastructure.yml','```','Replace the example path with this bundle location. Verify DNS, SSH host keys and TLS first.', 'On the controller, create the configured enrollment administrator once after checking existing users.','']
    steps += ['Copy each node manifest to its intended Ubuntu 24.04 amd64 machine. Install the project tooling there using docs/guided-setup.md. Confirm the controller with its operator.','From the project directory on that node:', '```sh', './rdc node check /ABSOLUTE/PATH/node-NAME.yml', './rdc node apply /ABSOLUTE/PATH/node-NAME.yml', 'sudo ./rdc node enroll /ABSOLUTE/PATH/node-NAME.yml', 'sudo ./rdc node status /ABSOLUTE/PATH/node-NAME.yml', '```', 'Applying requires sudo; enrollment requires independent controller administrator approval. Read-only commands never request sudo by themselves.', 'Do not copy tailscaled identity state between nodes. No application, backup or gateway is installed by this step.']

...
def envoy(profile,identity,peers,*,services=SUPPORTED):
    if not isinstance(services,tuple) or not set(services)<=set(('matrix','nextcloud')):raise ValueError('Unknown application route catalogue')
    ...
    for service in services:
        ...
        if service=='matrix':
            ...
            routes=[{'match':{'safe_regex':{'google_re2':{},'regex':pattern},'headers':[{'name':':method','string_match':{'safe_regex':{'google_re2':{},'regex':'('+'|'.join(methods)+')'}}}]},
                     'route':{'cluster':'local_'+service,'timeout':'120s'}} for methods,pattern in NEXTCLOUD_ROUTES]
...
def firewall(profile,peers,*,lan_interface,now,replace,services=SUPPORTED):
    ...
    sources=sorted({str(ipaddress.IPv4Address(profile['upstreams'][key])) for key in services if key in profile['upstreams']})
    members=[]
    for peer in peers:
        seconds=peer['expires_at']-now
        if seconds>0 and set(peer['services'])&set(services):members.append(str(ipaddress.IPv4Address(peer['address']))+' timeout '+str(seconds)+'s')

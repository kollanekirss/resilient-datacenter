...
def container_command(name,settings):
    ...
    return command+['--memory=256m','--cap-add=NET_BIND_SERVICE','--tmpfs','/config:rw,nosuid,nodev,size=16m',
                    '--tmpfs','/data:rw,nosuid,nodev,size=16m','--volume',str(regional.BASE/'Caddyfile')+':/etc/caddy/Caddyfile:ro',
                    '--volume',str(TLS)+':/tls:ro',image]


def regional_controls_command(settings,config):
    # The fixed PHP bridge accepts only values generated by the owned runtime.
...
def main():
    ...
    try:
        ...
        elif name=='proxy':regional.materialize(settings,(BASE/'Caddyfile').read_text(),(BASE/'config/config.php').read_bytes())

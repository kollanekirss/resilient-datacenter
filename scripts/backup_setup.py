...
def wizard(output,*,input_fn=input,output_fn=print):
    ...
    fields=[('institution_id','Institution/environment identifier'),('node_name','Source node name'),
            ('role','Source role: controller, relay, peer or portable'),('backup_host','Backup destination IPv4 or DNS name'),
    ...
    for field,label in fields:
        while True:
            ...
            elif field=='role' and value not in ('controller','relay','peer','portable'): output_fn('Choose one of the listed roles.');continue

...
def wizard(folder,*,input_fn=input,output_fn=print):
    ...
    try:
        ...
        while True:
            ...
            output_fn('Portable installation: 1 Preview | 2 Host check | 3 Allocate shells | 4 Fetch media | 5 Upload media | 6 Guest installation | 7 Instructions | 8 Local network | 9 Local applications | q Save and exit')
            choice=ask('Choose a step','q',input_fn=input_fn)
            if choice=='q':return {'state':'saved','plan':str(plan_path),'notice':'Progress retained. Local network preparation is step 8; local application preparation is step 9. Deployment and offline acceptance must be verified on the intended VMs.'}
            if choice=='9':
                from portable_application_commands import wizard_step
                try:wizard_step(plan,folder,input_fn=input_fn,output_fn=output_fn)
                except (ValueError,OSError) as error:output_fn('Application preparation needs attention: '+str(error))
                continue
            if choice=='8':

...
Before commit, migration runs on a copied candidate of the quiesced owned directories; the exact original directories remain untouched in private adjacent workspaces. Failure stops the candidate and atomically restores the original runtime/code/catalogue and database/configuration/files using the existing durable filesystem helpers. Never run the old application against a migrated database. This avoids the ordinary restore engine’s failure fallback, which would otherwise treat an upgraded database as a valid previous state. The upgrade journal must record rollback commitment before isolation is released, preventing a crash from causing a second stale rollback after users regain access. Interrupted recovery resumes the exact owned transaction; unknown journal fields, foreign state and changed binaries block automatic action.

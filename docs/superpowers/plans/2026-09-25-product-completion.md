...
11. Implemented in PR #12; both fresh-guest paths passed run 36165726657, with combined-source checks required before merge. Exercise installation behind simulated home NAT without inbound management SSH, then service use, encrypted remote backup and fenced replacement. Report the simulation boundary clearly; physical home routers, power/site independence and public DNS remain external acceptance items.
...
Private application ingress: PR #13 merged after all checks passed at e10133b. Matrix/Nextcloud packet probes reject routed LAN and spoofed overlay sources; allowed interface access, restart, application recovery, both upgrade paths and regional exchange pass. No other firewall table or backed-up application configuration was replaced.

Release preparation: current public guides now distinguish implemented capabilities from historical networking instructions. The repository has machine/access prerequisites, a role diagram, troubleshooting, an explicit real-site/unfamiliar-user worksheet and an enabled private vulnerability reporting channel. Candidate version is 0.3.0-alpha.1. Tagging/publication and actual provenance-verified download follow only after current source checks pass; real-site/provider/colleague acceptance remains NOT RUN.

Final default review: actual client logs showed upstream diagnostic uploads. The candidate sets the documented Tailscale opt-out in the managed systemd unit, preserving local logs and enrollment identity. A regression check requires the setting; fresh-guest acceptance additionally verifies it in the running daemon environment and repeats enrollment/recovery. Existing machines change only when the reviewed profile is reapplied. No global computer setting is modified.

...
- Operator-path review caught a missing administrator route: direct Proxmox access needs no gateway, but guest SSH requires explicit frontend/application/partner routes via the edge's actual management IP. The guide now lists them, plus the independent NTP device's return route. This changes guidance only; service configuration and test fixture topology are unchanged.

...
That single-host disposable fixture uses a synthetic networking daemon and does not demonstrate home NAT or physical separation. Real DNS/provider issuance, controlled application upgrades, regional federation and colleague usability remain outstanding. The optional issuer's frozen timer passed in [run 36133719119](https://github.com/kollanekirss/resilient-datacenter/actions/runs/36133719119): it activated a new trusted certificate and retained the active certificate after a simulated provider failure. That fixture cannot prove public ACME issuance. Do not use these results as a claim that the complete product is ready.
...
## One-time encryption keys during restoration

A physical database snapshot includes one-time keys that clients may consume after capture. Before the restored Synapse starts, the guarded recovery flow clears `e2e_one_time_keys_json` from the restored database. It leaves device identities, signing keys, media and encrypted room-key backups intact; clients can upload fresh one-time keys. If cleanup fails, candidate chat startup stops and the transaction follows its existing rollback/recovery path. This implements the [upstream Synapse recovery requirement](https://element-hq.github.io/synapse/latest/usage/administration/backups.html). An already committed restore is not truncated again when finishing an interrupted cleanup.

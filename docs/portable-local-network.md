...
The management address configured on OPNsense is `vms.edge.address`. The earlier site schema's `networks.management.gateway` remains a reserved address; this module does not configure it as another edge address. Management clients need no default route for their direct Proxmox connection. Other zones use their declared gateways.
...
Next comes the separate NGINX frontend and explicit portable application access profile, including backend TLS, trusted proxy settings, locally served Element assets, certificates and disconnected message/file operations. These are not implemented by preparing this network kit.

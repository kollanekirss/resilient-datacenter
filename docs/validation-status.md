...
The [controller run 36133848306](https://github.com/kollanekirss/resilient-datacenter/actions/runs/36133848306) accepted the guided HTTPS and backup grants through the actual pinned Headscale policy parser. This proves syntax/controller acceptance, not VPN packet enforcement. Local checks at this increment pass 410 tests and 16 playbook syntax checks.

A subsequent restart stress exercise is investigating an intermittent proxy restart failure observed in run 36132971329. Successful single runs do not close that finding. These fixtures also do not demonstrate real VPN enrollment, home NAT, physical site separation, Nextcloud, regional gateways or beginner acceptance. The older network-only statements above describe their historical increments.
